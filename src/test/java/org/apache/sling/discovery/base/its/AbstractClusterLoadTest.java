...
public abstract class AbstractClusterLoadTest {
        ...
        for (int i = 0; i < loopCnt; i++) {
            ...
            for (Iterator<VirtualInstance> it = instances.iterator(); it.hasNext(); ) {
                ...
                if (!instance.isViewCheckerRunning()) {
                    ...
                    new RetryLoop(
                            new ConditionImplementation(instance, aliveCntFinal),
                            INSTANCE_VIEW_TIMEOUT_SECONDS,
    ...
    class ConditionImplementation implements RetryLoop.Condition {
        ...
        public boolean isTrue() throws Exception {
            ...
            try {
                actualAliveCount = instance.getClusterViewService()
                        .getLocalClusterView()
                        .getInstances()
                        .size();
            ...
            if (!result) {
                logger.info("isTrue: expected=" + expectedAliveCount + ", actual=" + actualAliveCount + ", result="
                        + result);
            }
            return result;
        }

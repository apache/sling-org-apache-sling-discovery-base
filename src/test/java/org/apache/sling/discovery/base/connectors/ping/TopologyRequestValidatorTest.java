...
public class TopologyRequestValidatorTest {
    ...
    public void testTrustRequest() throws IOException {
        ...
        Assert.assertNotNull(
                method.getFirstHeader(TopologyRequestValidator.HASH_HEADER).getValue());
        Assert.assertTrue(method.getFirstHeader(TopologyRequestValidator.HASH_HEADER)
                        .getValue()
                        .length()
                > 0);
        Assert.assertNotNull(method.getFirstHeader(TopologyRequestValidator.SIG_HEADER));
        Assert.assertNotNull(
                method.getFirstHeader(TopologyRequestValidator.SIG_HEADER).getValue());
        Assert.assertTrue(method.getFirstHeader(TopologyRequestValidator.SIG_HEADER)
                        .getValue()
                        .length()
                > 0);
        final HttpServletRequest request = mock(HttpServletRequest.class);
        when(request.getHeader(TopologyRequestValidator.HASH_HEADER))
                .thenReturn(method.getFirstHeader(TopologyRequestValidator.HASH_HEADER)
                        .getValue());
        when(request.getHeader(TopologyRequestValidator.SIG_HEADER))
                .thenReturn(method.getFirstHeader(TopologyRequestValidator.SIG_HEADER)
                        .getValue());
        when(request.getHeader("Content-Encoding")).thenReturn("");
        when(request.getRequestURI()).thenReturn(method.getURI().getPath());
        when(request.getReader()).thenReturn(new BufferedReader(new StringReader(message)));

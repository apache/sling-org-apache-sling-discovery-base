...
public class TopologyConnectorClient implements TopologyConnectorClientInformation {
    ...
    TopologyConnectorClient(
            ...
            final AnnouncementRegistry announcementRegistry,
    ...
    void ping(final boolean force) {
        ...
        putRequest.setConfig(RequestConfig.custom()
        ...
        try {
            ...
            if (userInfo != null) {
                ...
                clientContext
                        ...
                        .setCredentials(
                                new AuthScope(
                                        putRequest.getURI().getHost(),
                                        putRequest.getURI().getPort()),
                                c);
            }
            ...
            if (response.getStatusLine().getStatusCode() == HttpServletResponse.SC_OK) {
                ...
                final String responseBody = requestValidator.decodeMessage(
                        putRequest.getURI().getPath(), response); // limiting to 16MB, should be way enough
                if (logger.isDebugEnabled()) {
                ...
                if (responseBody != null && responseBody.length() > 0) {
                    ...
                    if (inheritedAnnouncement.isLoop()) {
                        ...
                        if (inheritedAnnouncement.getOwnerId().equals(clusterViewService.getSlingId())) {
                            ...
                            if (config.isAutoStopLocalLoopEnabled()) {
                                inheritedAnnouncement =
                                        null; // results in connected -> false and representsloop -> true
                        ...
                        if (announcementRegistry.registerAnnouncement(inheritedAnnouncement) == -1) {
                            ...
                            statusDetails =
                                    "receiving side is seeing me via another path (connector or cluster) already (loop)";
                            return;
                        }
                    }
    ...
    private CloseableHttpClient createHttpClient() {
        ...
        builder.setDefaultSocketConfig(
                SocketConfig.custom().setSoTimeout(1000 * config.getSoTimeout()).build());
    ...
    public void disconnect() {
        ...
        // setting the connection timeout (idle connection, configured in seconds)
        deleteRequest.setConfig(RequestConfig.custom()
                .setConnectTimeout(1000 * config.getSocketConnectTimeout())
                .build());
        ...
        try {
            ...
            if (userInfo != null) {
                ...
                clientContext
                        ...
                        .setCredentials(
                                new AuthScope(
                                        deleteRequest.getURI().getHost(),
                                        deleteRequest.getURI().getPort()),
                                c);
            }
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "disconnect: done. code=" + response.getStatusLine().getStatusCode() + " - "
                                + response.getStatusLine().getReasonPhrase());
            }
